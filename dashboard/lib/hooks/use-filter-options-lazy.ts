...
export function useFilterOptions() {
  ...
  const [loadedOptions, setLoadedOptions] = useState<Record<string, FilterValue[]>>({})
  const { startDate, endDate } = useDateFilter()

  // Mapping of filter fields to their corresponding Tinybird pipe endpoints
  const fieldToPipeMap = useMemo<Record<FilterField, string>>(() => ({
  ...
  const staticOptions = useMemo<Partial<Record<FilterField, FilterValue[]>>>(() => ({
  ...
  const getFilterOptions = useCallback((field: FilterField, search?: string): FilterValue[] => {
    ...
    // For dynamic options, check cache first
    const cacheKey = `${field}_${startDate}_${endDate}_${search || ''}`
    const cached = optionsCache.get(cacheKey)
    if (cached && Date.now() - cached.timestamp < CACHE_DURATION) {
      return cached.data
    ...
    // Check if we have loaded options in state
    if (loadedOptions[field]) {
      return loadedOptions[field]
    }

    // Return empty array - options will be loaded asynchronously
    return []
  }, [startDate, endDate, staticOptions, loadedOptions])

  // Preload options for a specific field
  const loadFilterOptions = useCallback(async (field: FilterField, search?: string) => {
    await fetchFilterOptions(field, search)
  }, [fetchFilterOptions])
